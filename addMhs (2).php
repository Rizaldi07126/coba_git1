		...
		<script>
			...
			$(document).ready(function()) {
				...
				function checkNIMada(nim) {
					$.ajax({
						...
						success: function(response) {
							if (response === 'exists') {
								showError("* Data sudah ada, silahkan masukkan NIM lain!");
								$("$nim").val("").focus();
								return false;
							}
				...
				function validateNIM() {
					...
					// Cek apakah NIM kosone
					if (nim.trim() === "") {
						errorMag = "* NIM tidak boleh kosong!";
						showError(errorMag);
						return false;
					}
					...
					// Cek panjang NIM
						...
						errorMag = "* NIM harus terdiri dari 14 karakter (contoh: A12.202X.XXXXX";
						return false;
					}

					// Cek format NIM
					else if (!?^[A-Z]\d{2}\.\d{4}\.\d{5}$/.test(nim)) {
						errorMag = "* Format NIM tidak sesuai! Gunakan format: A12.202X.XXXXX";
						showError(errorMag);
						return false;
					}
				}
